...
pub fn main() {
    ...
    'running: loop {
        ...
        for source in &simulation.energy_nodes {
            canvas.filled_circle(source.0 as i16, source.1 as i16, 25, Color::RGB(0, 150, 0))
                .unwrap();
        }

        for node in &simulation.networks {
            canvas.filled_circle(node.x as i16,
                               node.y as i16,
                               25,
                               Color::RGB(if node.is_charging { 255 } else { 150 }, 0, 0))
                .unwrap();
            let start = (node.x as i32, node.y as i32);
            let diff = (node.facing.cos(), node.facing.sin());
            let end = ((node.x + diff.0 * 30f32) as i32, (node.y + diff.1 * 30f32) as i32);
